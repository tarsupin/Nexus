...
//		public List<MoonDataOld> moons;
//		public Vector2 textSize;

//		public PlanetDataOld(string name, string worldID, byte planetID, byte diff, byte[] icon) {
//			this.name = name;
//			this.worldID = worldID;
//			this.sprite = PlanetInfo.Planets[(byte)planetID];
//			this.diff = diff;
//			this.icon = icon;
//			this.moons = new List<MoonDataOld>();
//			this.textSize = Systems.fonts.baseText.font.MeasureString(name);
//		}

//		public void AddMoon(byte moonID) {
//			this.moons.Add(new MoonDataOld(moonID));
//		}
//	}

//	public class MoonDataOld {
//		public string sprite;
//		public short lat;
//		public float speed;
//		public float posX;
...
//		public MoonDataOld(byte moonID) {
...
//	public class PlanetSelectSceneOld : Scene {
...
//		// Planets
//		public Dictionary<short, PlanetDataOld> featured = new Dictionary<short, PlanetDataOld>();
//		public Dictionary<short, PlanetDataOld> planets = new Dictionary<short, PlanetDataOld>();

//		public PlanetSelectSceneOld() : base() {
...
//		public void LoadPlanets(string filename, Dictionary<short, PlanetDataOld> planetDict, short listID = 0) {
...
//			WorldListFormat planetDataOld = JsonConvert.DeserializeObject<WorldListFormat>(json);

//			// Load Planet Data
//			foreach(PlanetFormat planet in planetDataOld.planets) {

//				// Add Planet
//				planetDict.Add(listID, new PlanetDataOld(planet.name, planet.worldID, planet.planetID, planet.difficulty, planet.icon));
...
//		public void UpdateMoonPositions(PagingSystem paging, Dictionary<short, PlanetDataOld> planets) {
//			for(short i = paging.MinVal; i < paging.MaxVal; i++) {
//				PlanetDataOld planet = planets[i];

//				foreach(MoonDataOld moon in planet.moons) {
...
//				PlanetDataOld planet = this.featured[i];
...
//				PlanetDataOld planet = this.planets[i];
...
//		public void DrawPlanet(PlanetDataOld planetDataOld, short posX, short posY) {

//			// Draw Moons behind Planet:
//			foreach(MoonDataOld moon in planetDataOld.moons) {
//				if(!moon.front) { this.atlas.Draw(moon.sprite, posX + (short)moon.posX, posY + (short)moon.posY); }
//			}

//			// Draw Planet
//			this.atlas.DrawAdvanced(planetDataOld.sprite, posX + 2, posY, Color.White, 0f, 4);

//			// Display Name
//			Systems.fonts.baseText.Draw(planetDataOld.name, posX + 16 - (byte)Math.Floor(planetDataOld.textSize.X * 0.5f), posY + 78, Color.White);

//			// Display Difficulty
//			short diffSize = (short)Systems.fonts.console.font.MeasureString(GameplayTypes.DiffName[(byte)planetDataOld.diff]).X;
//			Systems.fonts.console.Draw(GameplayTypes.DiffName[(byte)planetDataOld.diff], posX + 16 - (byte)Math.Floor(diffSize * 0.5f), posY + 103, Color.White);

//			// Display Character
//			if(planetDataOld.icon != null && planetDataOld.icon[0] > 0 && planetDataOld.icon[1] > 0) {
//				Head.GetHeadBySubType(planetDataOld.icon[0]).Draw(false, posX - 30, posY - 45, 0, 0);
//				Suit.GetSuitBySubType(planetDataOld.icon[1]).Draw("StandLeft", posX - 30, posY - 45, 0, 0);

//				if(planetDataOld.icon[2] > 0) {
//					Hat.GetHatBySubType(planetDataOld.icon[2]).Draw(false, posX - 30, posY - 45, 0, 0);
//				} else if(planetDataOld.icon[0] == (byte)HeadSubType.PooHead) {
