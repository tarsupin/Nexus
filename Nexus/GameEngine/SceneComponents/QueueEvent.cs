...
namespace Nexus.Engine {
	...
	//		- Then add SetupTile(RoomScene room, short gridX, short gridY) to the tile.
	//			- See "Cannon" or "ChomperFire" for an example.
	public class QueueEvent {

		RoomScene room;
		Dictionary<int, List<short[]>> events;
		...
		public QueueEvent(RoomScene room) {
			this.room = room;
			this.events = new Dictionary<int, List<short[]>>();
		...
		public void AddEvent( int frame, byte tileId, short gridX, short gridY, short val1 = 0, short val2 = 0 ) {
		...
		private void RunEventsForThisFrame( int frame ) {
		...
		private void RunEvent( short tileId, short gridX, short gridY, short val1 = 0, short val2 = 0 ) {
			...
			tile.TriggerEvent(this.room, (short) gridX, (short) gridY, val1, val2);
		}
		...
		private bool RunBeatEvent( short tileId, short gridX, short gridY ) {
			var tileDict = Systems.mapper.TileDict;
			TileObject tile = tileDict[(byte) tileId];
			return tile.TriggerEvent(this.room, (short) gridX, (short) gridY);
		}
	}
}
