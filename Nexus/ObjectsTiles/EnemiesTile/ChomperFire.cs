...
namespace Nexus.Objects {
	...
	public class ChomperFire : TileGameObject {
		...
		public static void TileGenerate(LevelScene scene, ushort gridX, ushort gridY, byte subTypeId) {
			...
			if(!scene.IsClassGameObjectRegistered((byte) TileGameObjectId.ChomperFire)) {
				new ChomperFire(scene);
			}

			// Add to Tilemap
			scene.tilemap.AddTile(gridX, gridY, (byte) TileGameObjectId.ChomperFire, subTypeId);
		}

		public ChomperFire(LevelScene scene) : base(scene, TileGameObjectId.ChomperFire, AtlasGroup.Tiles) {
			this.collides = true;
			this.CreateTextures();
		}
