...
namespace Nexus.ObjectComponents {
	...
	public class WallJumpAction : ActionCharacter {
		...
		public void StartAction( Character character, DirCardinal dir, sbyte extraStrength = 0, byte extraDuration = 0, sbyte minimumDuration = 0 ) {
			...
			status.actionEnds = character.scene.timer.frame + stats.WallJumpDuration + extraDuration;
		...
		public override void RunAction( Character character ) {
			...
			if(!status.actionBool1 && character.scene.timer.frame > character.status.actionEnds - status.actionNum1) {
				this.EndAction(character);
				return;
			}
		}
